...
Reporte_Diario_VMMC_USAID <- read_excel("~/R/r_projects/Hfr/data_source/Reporte_Diario_VMMC_USAID.xlsx", .name_repair = "universal") # UTF ENCODING PRESERVES VARIABLE NAMES
...
em_vmmc <- Reporte_Diario_VMMC_USAID %>%
  ...
  dplyr::mutate(
    ...
    indicator_final = dplyr::case_when(grepl("T2_15.19.anos", indicator) ~ "HTS_TST",
                                       ...
                                       grepl("Complica??o", indicator) ~ "VMMC_AE")
  ) %>%
...
openxlsx::write.xlsx(em_vmmc, file = "~/R/r_projects/Hfr/output/em_vmmc.xlsx", sheetName = "em_vmmc")
openxlsx::write.xlsx(hfr_vmmc, file = "~/R/r_projects/Hfr/output/hfr_vmmc.xlsx", sheetName = "hfr_vmmc")
